...
- [] Main page (메인 페이지)
  - [] 전체 상품 리스트를 볼 수 있다.
    - [] 상품의 이름, 가격만 표시한다.
  - [] 상품 추가 버튼 클릭 시 상품 추가 페이지로 이동한다.
  - [] 상품 삭제 버튼 클릭 시 상품 삭제 확인 모달창을 띄운다.
  - [] 상품 수정 버튼 클릭 시 상품 수정 페이지로 이동한다.
  - [] 상품 클릭 시 상품 상세 보기 페이지로 이동한다.
  - [] 리스트 한 줄에 상품명, 가격, 수정버튼, 삭제버튼 있도록 구성한다.
  - [] 상품 리스트 10개씩 보이게하고, 10개 넘으면 다음 페이지에서 확인할 수 있도록 구성한다.
...
-[] ProductView page(상품 상세 정보 확인 페이지)
  - [] 상품 이름, 상품 가격, 상품 이미지, 상품 설명 표시한다.
  - [] 상품 수정 버튼 클릭시, 상품 수정 페이지로 이동한다.
  - [] 상품 삭제 버튼 클릭시, 상품 삭제 확인 모달창을 띄운다.

-[] ProductUpdate page(상품 정보 업데이트 페이지)
  - [] 원래의 상품 이름, 상품 가격, 상품 이미지, 상품 설명을 채워놓는다.
  - [] 상품 이름, 가격, 이미지, 설명을 수정할 수 있도록 한다.
  - [] 상품 수정 완료 버튼을 누르면, 상품 상세 페이지로 넘어간다.
  - [] 상품 수정 취소 버튼을 누르면, 이전 페이지로 돌아간다.
