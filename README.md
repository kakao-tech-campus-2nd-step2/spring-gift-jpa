...
- [ ] 상품 이름 길이 검증
  - [ ] 상품 이름은 최대 15자까지 입력할 수 있도록 검증
  - [ ] 길이가 15자를 초과하는 경우 에러 메시지 반환

- [ ] 상품 이름 특수 문자 검증
  - [ ] 허용된 특수 문자 ( ), [ ], +, -, &, /, _ 만 사용할 수 있도록 검증
  - [ ] 허용되지 않은 특수 문자가 포함된 경우 에러 메시지 반환

- [ ] "카카오" 포함 문구 검증
  - [ ] "카카오"가 포함된 문구는 담당 MD와 협의한 경우에만 사용할 수 있도록 검증
  - [ ] "카카오"가 포함된 경우 에러 메시지 반환

- [ ] 잘못된 입력 값에 대한 에러 메시지 반환
  - [ ] 각 검증 단계에서 발생한 에러에 대한 정보를 클라이언트에게 제공
  - [ ] 예를 들어, 상품 이름 길이가 초과된 경우 "상품 이름은 최대 15자까지 입력할 수 있습니다."와 같은 메시지 반환
...
- [ ] 예외 처리 및 응답 포맷팅
  ...
  - [ ] 에러 메시지를 일관된 포맷으로 제공하여 클라이언트가 이해하기 쉽도록 함
