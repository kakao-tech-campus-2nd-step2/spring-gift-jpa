...
public class WishService {
    ...
    public void addWish(Member member, RequestWishDTO requestWishDTO) {
        Wish wish = new Wish(member.getEmail(), requestWishDTO.getProductId(), requestWishDTO.getCount());
        WishRepository.save(wish);
    }

    public List<ResponseWishDTO> getWish(Member member) {
        return WishRepository.findWishsByEmail(member.getEmail());
    }

    @Transactional
    public List<ResponseWishDTO> editWish(Member member, RequestWishDTO requestWishDTO) {
        Wish wish = WishRepository.findByEmailAndProductId(member.getEmail(), requestWishDTO.getProductId())
